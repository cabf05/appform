...
def generate_participant_link(table_name, user_id=None):
    """Generates a link for participants to access the meeting, optionally with user_id."""
    base_url = "https://app-number.streamlit.app"
    if user_id:
        return f"{base_url}/?table={table_name}&mode=participant&user_id={user_id}"
...
# Initialize user_id from query params if provided, otherwise generate a new one
...
if mode == "participant" and table_name_from_url:
    ...
    # Show the persistent link with user_id for this participant
    participant_link = generate_participant_link(table_name_from_url, user_id)
    st.markdown(f"**Your Persistent Link:** [{participant_link}]({participant_link})")
    st.write("Save this link to always access your same number!")
    ...
    try:
        ...
        if existing.data:
            ...
            # Assign a new number if none exists
        ...
        # Display the number (either existing or newly assigned)
        st.markdown(f"""
