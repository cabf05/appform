...
def create_meeting_table(supabase, table_name, meeting_name, max_number=999):
    ...
    try:
        ...
        if response_metadata.status_code not in [200, 201]:
            st.error(f"Erro ao registrar metadados: {response_metadata.data}")
            return False
        
        # Passo 2: Criar a estrutura da tabela dinamicamente via SQL
        ...
        if response_create.status_code not in [200, 201]:
            st.error(f"Erro ao criar tabela {table_name}: {response_create.data}")
            supabase.table("meetings_metadata").delete().eq("table_name", table_name).execute()  # Rollback
            return False
        ...
        for i in range(0, max_number, batch_size):
            ...
            if response_insert.status_code not in [200, 201]:
                st.error(f"Erro na inserção de dados (batch {i}-{end}): {response_insert.data}")
                supabase.table(table_name).rpc("execute_sql", {"query": f"DROP TABLE public.{table_name}"}).execute()
                supabase.table("meetings_metadata").delete().eq("table_name", table_name).execute()  # Rollback
                return False
        st.success(f"Tabela {table_name} criada e populada com sucesso!")
        return True
    except Exception as e:
        st.error(f"Erro geral ao criar tabela da reunião: {str(e)}")
        return False
...
**Importante**: Crie a seguinte tabela no seu Supabase:
